...
export async function ensureSchema() {
  ...
  // support_identity_reveals is dropped, not created.
  //
  // It was the audit trail for a support queue that showed customers under a
  // pseudonym and made a member of staff ask before seeing the real name. That
  // design was built the wrong way round and was reversed: staff see the
  // customer, and it is the staff member who is concealed — every reply comes
  // from "Taxify Support". There is nothing left to reveal, so nothing to
  // record, and the table has never held a row.
  await pool.query('DROP TABLE IF EXISTS support_identity_reveals');

  // What the site is actually doing, one row per view or click.
  //
  // Deliberately its own table and deliberately narrow. Everything in it is
  // either derived from headers the browser volunteered or generated by us —
  // there is no IP address and no name, because a table that answers "how many
  ...
  //
  // Indexed on (at) first because every question here starts with a date
  // range, and separately by the three things the range is then grouped by.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS page_events (
      id BIGINT PRIMARY KEY AUTO_INCREMENT,
      at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      surface VARCHAR(12) NOT NULL,
      event VARCHAR(40) NOT NULL DEFAULT 'view',
      path VARCHAR(160) NOT NULL,
      label VARCHAR(80) NULL,
      visitor CHAR(32) NULL,
      is_new TINYINT(1) NOT NULL DEFAULT 0,
      referrer_kind VARCHAR(12) NULL,
      ...
      KEY idx_events_surface_at (surface, at),
      KEY idx_events_event_at (event, at),
      KEY idx_events_visitor (visitor, at),
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
  `);
  ...
  // A year and no more.
  //
  // One row per view is the finest grain there is, and the questions this
