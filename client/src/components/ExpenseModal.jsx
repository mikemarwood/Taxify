...
// A fact about the expense. The glyph is what lets the six of them be found
// by shape instead of read in order — which is how a record like this is
// actually used, looking for one thing rather than all of it.
function DetailRow({ icon, label, value }) {
  return (
    <div className="xd-row">
      {icon && (
        <span className="xd-mark">
          <Icon name={icon} size={16} />
        </span>
      )}
      <div style={{ minWidth: 0 }}>
        <div className="xd-label">{label}</div>
        <div className="xd-value">{value}</div>
      </div>
    </div>
  );
}
          ...
          {editing ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div className="xd-grid">
                ...
                />
                <DetailRow icon="tag" label="Category" value={expense.category?.name || 'Uncategorised'} />
                <DetailRow
                  icon="repeat"
                  label="Recurring"
                  value={
                ...
                />
                {expense.autoGenerated && <DetailRow icon="bolt" label="Added automatically" value="Yes" />}
                {expense.createdAt && (
                  <DetailRow
                    icon="calendar"
                  label="Added on"
                ...
                {expense.createdByName && <DetailRow icon="user" label="Added by" value={expense.createdByName} />}
              ...
              <div>
                <div className="xd-section-head">
                  ...
                  <span className="xd-label">Receipt</span>
                  {/* Beside the heading rather than under the picture. The
                      preview is already clickable, but nothing said so. */}
                  {expense.receiptUrl && (
                    <button
                      type="button"
                      className="btn btn-ghost"
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 4, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                  {!readOnly && (
                    <>
                      ...
                      {/* Red on the word and the glyph, not a red fill. A solid
                          red button beside a solid blue one reads as the pair
                          of them being equally likely, and this one is not. */}
                      <button
                        className="btn btn-ghost"
                        style={{ color: 'var(--red)' }}
